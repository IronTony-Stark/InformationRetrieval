...
class BooleanSearch(inputQuery: String, private val dict: Dict, private val dictBiword: DictBiword) {
    ...
    private fun word(): List<Int> {
        ...
        when (next) {
            ...
            '\"' -> {
                ...
                } else throw SyntaxException("Expecting NOT or 'word', found $word")
            }
        }
    }
    ...
    private fun operator(word1: List<Int>): List<Int> {
        ...
        return if (operator == "AND" || operator == "OR") {
            ...
            operator(result)
        } else if (operator != "") throw SyntaxException("Expecting AND OR, found $operator")
        else word1
    }
    ...
    private fun phraseQuery(): List<Int> {
        ...
        while (next != '\"') {
            if (next == '$')
                throw SyntaxException("\" is missing")
    ...
    private fun match(c: Char) {
        next = if (next == c) input.nextChar() else throw SyntaxException("Expecting $c, found $next")
    }
